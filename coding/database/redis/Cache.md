...
> 在实际使用中，Redis一般和其他数据库搭配使用，用来减轻后端数据库的压力，比如和关系型数据库Mysql配合使用。

Redis作为缓存的流程

[Redis Cache Diagram](./assets/RedisCache.drawio ":include :type=code")
...
[Empty Cache Diagram](./assets/CacheEmpty.drawio ":include :type=code")
...
- 布隆过滤器
    ...
    首先将用户可能会访问的热点数据存储在布隆过滤器中（也称`缓存预热`），当有一个用户请求到来时会先经过布隆过滤器，如果请求的数据，布隆过滤器中不存在，那么该请求将直接被拒绝，否则将继续执行查询。

    > 缓存预热：是指系统启动时，提前将相关的数据加载到 Redis 缓存系统中。这样避免了用户请求的时再去加载数据。

[Cache Bloom filter Diagram](./assets/CacheBloom.drawio ":include :type=code")
...
- 分布式锁
    ...
    - 锁：当我们通过 key 去查询数据时，首先查询缓存，如果没有，就通过分布式锁进行加锁，第一个获取锁的进程进入后端数据库查询，并将查询结果缓到Redis 中。
